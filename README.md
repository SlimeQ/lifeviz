...
- The main output window now uses custom chrome instead of the native Windows title bar. Dragging the background anywhere in the output window now moves the window, double-clicking that background toggles fullscreen, and resizing from the left/right/bottom hit zones stays inside LifeViz pointer handling instead of entering the OS move/resize loop. The top chrome buttons expose the root menu (`...`), Scene Editor (`SE`), minimize, fullscreen, and close.
...
- File-backed smoke profiling is now supported: `--smoke-test profile-file-240 <video>` and `profile-file-480 <video>` run the real frame profiler against an actual media file source, which is the right way to measure decode/playback regressions that synthetic scenes cannot reproduce.
