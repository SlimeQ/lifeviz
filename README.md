...
`gpu-benchmark` now reports both the original GPU sim/source timings and the direct GPU composite-to-simulation handoff timings, so you can see whether source readback is still present. `gpu-handoff` validates that a GPU-built composite can inject directly into the grayscale GPU simulation path without a CPU composite readback. `gpu-sim` validates the D3D11 grayscale simulation backend plus CPU fallback handoff. `gpu-source` drives the real `BuildCompositeFrame` path through `MainWindow` with synthetic sources and fails unless the GPU source compositor actually runs. `gpu-render` launches a hidden `MainWindow` and fails unless the real GPU composite pipeline initializes through the app render path. `shutdown` now opens the Scene Editor and then closes the real main window, failing if shutdown teardown captures any exception or if the owned editor-close path throws. `startup` launches a minimal WPF window in smoke-test mode without loading the persisted project, media sources, ffmpeg decode, or audio loopback so UI/render initialization can be tested in isolation. `all` runs `gpu-sim` plus the combined GPU handoff/source/render UI smoke suite.
`dimensions` applies a real simulation height/depth change through `MainWindow` and fails unless both the active simulation backend and presentation surface resize to match.
...
- Simulation layers now sit behind an `ISimulationBackend` seam, and the default backend is now `GpuSimulationBackend` when possible. The first GPU path currently supports the `Naive Grayscale` life mode: it runs injection, Conway stepping, depth-history shifting, and color-buffer generation in D3D11 compute shaders. `RGB Channel Bins` still falls back to `CpuSimulationBackend` automatically, so the scene/editor/config plumbing can stay stable while GPU coverage expands.
- Runtime smoke tests are now first-class: `--smoke-test gpu-benchmark` profiles the current GPU sim/source stack, `--smoke-test gpu-handoff` verifies direct GPU composite-to-sim injection without CPU composite readback, `--smoke-test gpu-sim` exercises the GPU simulation backend directly, `--smoke-test gpu-source` verifies that `MainWindow` really uses the D3D11 source compositor for `BuildCompositeFrame`, `--smoke-test gpu-render` verifies that `MainWindow` really brings up the D3D11 presentation composite path, `--smoke-test shutdown` verifies that the real window can tear down cleanly even when the Scene Editor is open, and `--smoke-test startup` opens the app in an isolated smoke-test mode that skips persisted scene/config startup so renderer/UI regressions can be caught without spinning up user media pipelines.
- The main frame loop now runs from a background-priority WPF `DispatcherTimer` instead of `CompositionTarget.Rendering`. It still shares the UI thread, but input/title-bar/menu work can preempt it more cleanly. LifeViz still throttles simulation/render further while the root context menu is open, while the window is being dragged/resized through the native move loop, and while the Scene Editor is the active window.
- Naive-grayscale GPU simulation now shares the same D3D11 device/context as the GPU source compositor, so grayscale layers can inject straight from the final GPU composite texture. When passthrough underlay and recording do not need a CPU composite copy, the source-composite readback is skipped for that frame.
...
- Settings persist to `%AppData%\lifeviz\config.json` after the app finishes loading (height/rows, depth, framerate, blend/composite toggles, full simulation-layer stack settings including per-layer thresholds, opacity, RGB hue shift offset/speed, passthrough, audio reactivity controls, etc.) and restore on next launch. The window keeps the current aspect ratio and can be resized from the corner grip; use height presets or fullscreen to change simulation resolution without letterboxing.
