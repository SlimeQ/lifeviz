...
- **Scene Editor...** - opens a dedicated source-management window with a draggable nested tree on the left and selected-layer settings on the right. It exposes the same source operations as the *Sources* context menu (add/remove/reorder/nest sources, set primary, rename groups, restart video layers, pause/play, scrub, tune blend/fit/opacity/video-audio/video-audio-volume/keying, and manage animations). For video-capable layers, the right pane adds a scrub slider with current/total time and a play/pause button. The header includes global source-audio controls: *Master Audio* (toggle) plus a master volume slider that scales/mutes all source audio. It also includes **App Controls...**, which opens the full main context menu at the Scene Editor location so global controls (simulation/audio/render/system toggles) are available without switching screens. Toggle *Live Mode* to apply per-layer edits immediately, or turn it off and use *Apply* to commit a batch of edits at once. Header master-audio controls apply immediately.
...
- Height, depth, thresholds, modes, opacity, RGB hue shift offset/speed, framerate, blend/passthrough toggles, per-video audio toggles, per-video audio volume, global source-audio master toggle/volume, and audio reactivity settings persist to `%AppData%\lifeviz\config.json` after the app finishes loading, so startup control events no longer overwrite prior configs.
