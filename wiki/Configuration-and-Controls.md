...
- Height, depth, thresholds, modes, opacity, framerate, and blend/passthrough toggles persist to `%AppData%\lifeviz\config.json` after the app finishes loading, so startup control events no longer overwrite prior configs.
