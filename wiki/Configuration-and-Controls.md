...
- **Show FPS** - toggles an overlay with `Render fps`, `Sim sps`, target simulation FPS, frame-pacing diagnostics, a live `Work:` stage breakdown (`frame`, `composite`, `inject`, `sim`, `render`), and audio/reactivity diagnostics (input gain, multiplier, beat count, and last-step seed burst counts). Audio stats include a signal state (`Signal` vs `Low/No Signal`) to make input troubleshooting clearer.
