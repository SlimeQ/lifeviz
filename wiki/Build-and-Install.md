...
- `startup` launches `MainWindow` in a dedicated smoke-test mode that skips loading the persisted project plus file/video/audio capture pipelines, so WPF/render startup can be validated in isolation and should exit quickly. It also verifies the per-user first-run decision: a missing/unusable config produces a starter `Sim Group` with a runtime `Life Sim`, while a successfully loaded empty scene remains empty.
