...
- `sim-group-inline-presentation` drives a real inline `Sim Group` scene through the GPU `DrawingSurface`, reads back the actual presented pixels, and fails unless those pixels stay synchronized with the evolving inline composite. This is the targeted stale-shared-texture regression smoke for the inline sim-group present path.
